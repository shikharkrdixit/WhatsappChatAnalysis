...
chat <- rwa_read("./WhatsApp Chat with Srivathsan SRM.txt") %>%  filter(!is.na(author))
head(chat,3)
...
to_remove <- c(stopwords(kind = 'en'),"media","omitted","ref","3","yes","yess","stu","us","go",
...
library(wordcloud)
wordcloud(chatx$word,max.words = 50,random.order = F,colors=brewer.pal(8,"Dark2"),scale=c(2,1))
